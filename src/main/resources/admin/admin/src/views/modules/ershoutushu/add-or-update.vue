    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="成色" prop="chengse">
          <el-select :disabled="ro.chengse" v-model="ruleForm.chengse" placeholder="请选择成色">
            <el-option
...
export default {
  ...
  methods: {
	...
	this.ruleForm.tushujianjie = this.ruleForm.tushujianjie.replace(reg,'../../../springbootoc81w/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `ershoutushu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.ershoutushuCrossAddOrUpdateFlag = false;
						 this.parent.search();
